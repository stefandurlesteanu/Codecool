...
def choose():
    crm_option = ui.get_inputs(["Please select option: "], "")[0]
    if crm_option == "1":
        show_table(table)
        return False
    elif crm_option == "2":
...
def add(table):
    ...
    """
    title_input = "Please insert the details of new customers: "
...
def get_longest_name_id(table):
    """
        ...
        """
    longest_name = len(table[0][1])
    result = []
    for row in table:
        if len(row[1]) >= longest_name:
            result.append(row)
...
def get_subscribed_emails(table):
    ...
    result_2 = []
    for row in result:
        result_2.append(row[1] + separator + row[0])
    return result_2
